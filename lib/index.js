...
const TileView = ({
  ...
  const calcTileDimensions = (deviceWidth, tpr) => {
    ...
    if (tpr === 2) {
      ...
      margin = deviceWidth / (4 * 10);
        ...
        margin = deviceWidth / (tpr * 7);
    } else if (tpr === 6) {
        margin = deviceWidth / (tpr * 6);
    } else if (tpr === 7) {
        margin = deviceWidth / (tpr * 5);
    } else if (tpr === 8) {
        margin = deviceWidth / (tpr * 4);
    } else if (tpr === 9) {
        margin = deviceWidth / (tpr * 3);
    } else if (tpr > 9) {
        margin = deviceWidth / (tpr * 2);
    }
  ...
  const TileItem = ({size, margin, touchable, data, index, obj, tileHeight, tileStyle}) => (
    <View key={index}>
      {touchable && touchable === true && (
        <TouchableOpacity onPress={() => touchableCallback(index, obj)}>
          <View
            style={[
              tileStyle ? tileStyle : currentStyle.item,
              {width: size, height: tileHeight ? tileHeight : size, marginHorizontal: margin},
            ]}>
            {tileContent(data)}
          </View>
        </TouchableOpacity>
